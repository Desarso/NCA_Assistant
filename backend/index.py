...
async def get_current_user(
    ...
    try:
        ...
        raise HTTPException(
            status_code=401, detail=f"Failed to validate Firebase ID token: {str(e)}"
        )
...
def dict_to_model_message(data: dict) -> Union[ModelRequest, ModelResponse]:
    ...
    if data["type"] == "model_request":
        return ModelRequest(parts=parts, kind=data.get("kind"))
    else:  # model_response
...
def read_user(
    ...
    if user_id != current_user.uid:
        raise HTTPException(
            status_code=403,
            detail="Access denied: You can only view your own user data",
        )
